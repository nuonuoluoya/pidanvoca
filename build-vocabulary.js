...
<body data-study-mode="full">
  ...
  <script>
    ...
    const memoryMetaStore = 'reviewMeta';
    ...
    async function exportMemoryProgress() {
      try {
        const [reviewCards, reviewLogs, metaValues] = memoryStorageAvailable ? await Promise.all([
          memoryReadAll(memoryCardStore), memoryReadAll(memoryLogStore),
          (async () => {
            const database = await openVocabularyDatabase();
            const transaction = database.transaction(memoryMetaStore, 'readonly');
            const store = transaction.objectStore(memoryMetaStore);
            const keys = await memoryRequest(store.getAllKeys());
            const values = await memoryRequest(store.getAll());
            return keys.map((key, index) => [key, values[index]]);
          })()
        ]) : [Array.from(memoryVolatileCards.values()), Array.from(memoryVolatileLogs.values()), [['memory-settings', { dailyNew: memoryDailyNew }]]];
    ...
    async function importMemoryProgress(file) {
      ...
      try {
        ...
        const database = await openVocabularyDatabase();
        const transaction = database.transaction([memoryCardStore, memoryLogStore, memoryMetaStore], 'readwrite');
        const cardStore = transaction.objectStore(memoryCardStore);
        const logStore = transaction.objectStore(memoryLogStore);
        const metaStore = transaction.objectStore(memoryMetaStore);
        if (replace) {
          cardStore.clear();
          logStore.clear();
          metaStore.clear();
        ...
        for (const card of payload.reviewCards) {
          if (!card || typeof card.cardId !== 'string' || typeof card.bookId !== 'string') continue;
          if (replace) cardStore.put(card);
          else {
            const existing = await memoryRequest(cardStore.get(card.cardId));
            if (!existing || Number(card.updatedAt) >= Number(existing.updatedAt)) cardStore.put(card);
          }
        }
        payload.reviewLogs.forEach((log) => { if (log && typeof log.logId === 'string') logStore.put(log); });
        payload.metaEntries.forEach((entry) => { if (Array.isArray(entry) && entry.length === 2) metaStore.put(entry[1], entry[0]); });
        await memoryTransactionDone(transaction);
    ...
    async function resetMemoryProgress(bookId) {
      ...
      try {
        ...
        if (!bookId) {
          ...
          const cards = await memoryRequest(cardStore.getAll());
          const logs = await memoryRequest(logStore.getAll());
          const metaKeys = await memoryRequest(metaStore.getAllKeys());
          cards.filter((record) => record.bookId === bookId).forEach((record) => cardStore.delete(record.cardId));
          logs.filter((record) => record.bookId === bookId).forEach((record) => logStore.delete(record.logId));
          metaKeys.filter((key) => String(key).startsWith('daily:' + bookId + ':')).forEach((key) => metaStore.delete(key));
        }
        await memoryTransactionDone(transaction);
        invalidateMemorySessionHistory(bookId);
        if (!bookId) {
